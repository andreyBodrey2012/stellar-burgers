...
describe('проверка feed стора', () => {
  ...
  it('проверка получения заказов ленты заказов', async () => {
    const feedOrder = {
      success: true,
      orders: [
        {
    ...
    global.fetch = jest.fn(() =>
      Promise.resolve({
        ok: true,
        json: () =>
          new Promise((resolve) => setTimeout(() => resolve(feedOrder), 0))
      })
    ) as jest.Mock;
    ...
    const store = configureStore({
      reducer: { feedOrder: feedOrdersReducer }
    });
    ...
    const { items, error } = store.getState().feedOrder;
    expect(error).toBeNull();
    expect(items).toEqual(feedOrder.orders);
  });
});
